...
class VideoClassifierDev:
    ...
    def _check_fr_similarity(self, face: Dict[str, Any]) -> List[np.ndarray]:
        """
        ...
        """
        if len(self.embedding_live_face_list) == 0:
            self.embedding_live_face_list.append(face['embedding'])
            print('new')
            return self.embedding_live_face_list, False
        print(face.keys())
        if 'embedding' in face:
            ...
            similar_embeddings = [emb for emb in self.embedding_live_face_list if self._is_embedding_similar(emb, embedding)]
            if similar_embeddings:
                print('similar')
                return self.embedding_live_face_list, True
            else:
                print('not similar')
                self.embedding_live_face_list.append(embedding)
        return self.embedding_live_face_list, False
    ...
    def _classify_multiple_live_faces(self, 
        ...
        if mode.lower() == "rc1":
            for idx, frame in enumerate(frames_list):
                if len(frame['faces']) > 1:
                    return {'contain_multiple_live_faces': True,
                        'n_faces': len(frame['faces']),
                        'class': 1,
                        #'frames_idx_list': frames_list, #only idx for analysis in the future
                        }
        elif mode.lower() in ["rc4","rc3", "rc5","rc6"]:
            for idx, frame in enumerate(frames_list):
                #if idx == 0:
                #    embedding_reference_list.append(frame['faces'][0]['embedding'])

                #pdb.set_trace()
                card_bboxes = self._get_card_bounding_boxes(frame['frame'])
                temp_detail = {'n_face': len(frame['faces']),
                            'n_card': len(card_bboxes)}
                if mode == "rc3":
                    number_live_faces = temp_detail['n_face'] - temp_detail['n_card']
                elif mode in ["rc4", "rc5","rc6"]:
                    ...
                    if mode == 'rc6':
                        for idx_j, face in enumerate(frame['faces']):
                            if idx_j not in not_live_face_idx:
                                ...
                                occluded_face = self._check_face_occlusion(face['bbox'], card_bboxes, threshold=0.7)
                                if not occluded_face:  # Only process non-occluded faces
                                    self.embedding_live_face_list, flag_similar = self._check_fr_similarity(face)
                                    
                print('n embedding live',len(self.embedding_live_face_list))
                number_live_faces = max(number_live_faces, len(self.embedding_live_face_list))
                temp_detail['number_live_faces'] = number_live_faces
                if self.visualize:
        ...
        #logger.info(f"Reference embedding: {embedding_reference_list[0][:10]}...")
        return {
    ...
    def _check_face_occlusion(self, face_bbox, card_bboxes, threshold=0.7):
        ...
        print(f"Face visible percentage: {visible_percentage:.3f}")
    ...
    def classify_video(self, video_path: str, 
        ...
        self.embedding_live_face_list = [] #np.array([0.0] * 512)  # Placeholder for reference embedding
        try:
            # Get video properties
            video_info = self._extract_video_info(cap, video_path)
            logger.debug(f"Processing video: {video_info['filename']}")
            ...
            logger.info(f'TOTAL FLAGGED FRAMES: {len(flagged_frames)}')

            video_classification = self._classify_multiple_live_faces(flagged_frames)
            print(f"Video classification result: {video_classification['contain_multiple_live_faces']}")
            ...
            logger.info(f"contain multiple live faces: {video_classification['contain_multiple_live_faces']}")
            logger.info(f"class: {video_classification['class']}")
            logger.info(f"n_faces: {video_classification['n_faces']}")
            #logger.info(f"Video processing completed in {results['timing']['total_processing']:.3f}s")
            #logger.info(f"Detection summary: {analysis['summary']}")
